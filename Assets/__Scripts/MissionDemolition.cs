...
public class MissionDemolition : MonoBehaviour
{
    ...
    void NextLevel()
    {
        if (level + 1 == levelMax) // Only show Game Over at the last level
        {
            gameOverManager.ShowGameOver();
        }
        else
        {
            level++;  // Increment level only if it's not the last one
            StartLevel();
        }
    }
    ...
    {
        level = 0;  // Reset level count when the game restarts
        shotsTaken = 0;  // Reset shot count
        StartLevel();  // Start the first level again
    }
}
